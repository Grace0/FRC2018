...
void DriveControllerMother::AutonWrapper(
	...
	while (frc::RobotState::IsAutonomous() && frc::RobotState::IsEnabled()) {
		...
		if (timerAuton->HasPeriodPassed(DRIVE_WAIT_TIME)) {
			...
			for (unsigned int i = 0; i < sizeof(drive_ref); i++) { //looks through each row and then fills drive_ref with the column here, refills each interval with next set of refs
